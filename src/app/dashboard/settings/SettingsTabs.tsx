      ...
      <div className="p-8">
        ...
        {activeTab === "subscription" && (
          <div className="space-y-6">
            ...
            {(() => {
              const activeDiscounts = vendor.discounts?.filter((d: any) => {
                const now = new Date();
              ...
              return (
                <div className="mb-8 p-6 bg-blue-50 border border-blue-200 rounded-2xl shadow-sm">
                  ...
                  <p className="text-sm text-blue-800 mb-4">A központ által beállított egyedi, kedvezményes díjak a fiókodhoz. (Ahol nincs külön kiemelve, ott az alapértelmezett csomagdíj érvényes.)</p>
                  
                  <div className="space-y-3">
                    {activeDiscounts.map((d: any) => {
                      const options = [
                        { label: "Jutalék", value: d.discountedCommissionRate, suffix: "%" },
                        { label: "Akciós Jutalék", value: d.discountedPromoCommissionRate, suffix: "%" },
                        { label: "Havidíj", value: d.discountedMonthlyFee, suffix: " Ft", format: true },
                        { label: "Marketing Hozzájárulás", value: d.discountedMarketingFee, suffix: "%" },
                        { label: "Bankkártyás Díj", value: d.discountedCardFee, suffix: "%" },
                      ].filter(o => o.value !== null);

                      if (options.length === 0) return null;

                      return (
                        <div key={d.id} className="bg-white/60 p-4 rounded-xl border border-blue-100">
                          {d.endDate && (
                            <div className="text-xs font-bold text-red-500 mb-3 bg-red-50 inline-block px-2 py-1 rounded">
                              Érvényes eddig: {new Date(d.endDate).toLocaleDateString('hu-HU')}
                            </div>
                          )}
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                            {options.map((opt, idx) => (
                              <div key={idx}>
                                <p className="text-xs font-semibold text-blue-800/60 uppercase tracking-wider mb-1">{opt.label}</p>
                                <p className="font-bold text-blue-700 text-lg">
                                  {opt.format ? opt.value.toLocaleString('hu-HU') : opt.value}{opt.suffix}
                                </p>
                              </div>
                            ))}
                          </div>
